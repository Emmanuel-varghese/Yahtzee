...
function yahtzeeGame () {
	...
	function kind(kindVal) {
		fetch('/kind', {
		  ...
		  body: JSON.stringify({
			kindVal: kindVal,
			dice: Array.from({ length: 5 }, (_, i) => Number(document.getElementById(i).innerHTML)),
		  }),
		})
		  .then(response => response.json())
		  .then(data => {
	...
	function checkStraight(isSmallStraight) {
		fetch('/check-straight', {
		  method: 'POST',
		  headers: {
			'Content-Type': 'application/json',
		  },
		  ...
		  .then(response => response.json())
		  .then(data => {
			if (data.straightAchieved) {
			  console.log(isSmallStraight ? 'Small straight achieved!' : 'Large straight achieved!');
			  
			} else {
			  console.log(isSmallStraight ? 'Small straight not achieved.' : 'Large straight not achieved.');
	  ...
	  document.getElementById("smallStraight").onclick = () => checkStraight(true);
		document.getElementById("largeStraight").onclick = () => checkStraight(false);
